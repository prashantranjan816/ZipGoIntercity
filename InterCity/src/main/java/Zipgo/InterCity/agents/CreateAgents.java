...
public class CreateAgents extends LoginPage {

	final static By agents = By.xpath("//a[text()='Agents']");
	final static By createAgent = By.xpath("//a[text()='Create New Agent']");
	String name;

	@Test(priority = 1, invocationCount = 1, enabled = true)
	public void createAgents() throws IOException, InterruptedException {
		try {
			...
			driver.findElement(createAgent).click();
			Thread.sleep(3000);
